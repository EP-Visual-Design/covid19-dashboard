...
export default function extract_slices(
  ...
  items.forEach((item, index) => {
    const yvalue = item[sumFocus][yprop];
    stats_total += yvalue;
    if (index < slideIndex) {
      upper_stat += yvalue;
      upper_count += 1;
  ...
  if (upper_count > 0) {
    ...
    slices.splice(0, 0, { x, y: 0, oy: y, label, count: upper_count });
  }
  if (other_count > 0) {
    const x = lower_label;
    const y = other_stat;
    const label = '[' + other_count + '] ' + x + '\n' + countFormat(y);
    // slices.push({ x, y: 0, oy: y, label, count: other_count });
    slices.push({ x, y: y, label, count: other_count });
  }
  if (formatPercent) {
    slices.forEach((item) => {
      let oy = item.oy;
